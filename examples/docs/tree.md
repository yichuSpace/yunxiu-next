...
<template>
  <div flex>
    <div style="width: 300px;">
      <yun-tree :data="data" @select-change="handleSelect" :lock-select="lockSelect" title-ellipsis></yun-tree>
    </div>
    <div style="width: 80px">
      <yun-switch v-model="lockSelect" size="large">
            ...
            children: [
              {
                title: '二级 1-1',
                expand: true,
                children: [{ title: '三级 1-1-1 我是超长字段我是超长字段我是超长字段' }, { title: '三级 1-1-2' }],
              },
              {
                title: '二级 1-2',
                expand: true,
...
```html
<template>
  <yun-button @click="initData">初始化数据并默认选中前端组</yun-button>
  <div flex class="mt-10">
    <div style="width: 300px">
      <yun-tree :data="data" ref="tree" @select-change="handleSelect" titleKey="text"></yun-tree>
    </div>
  </div>
  <div></div>
</template>
...
<template>
  <div flex>
    <div class="p10" style="width: 300px; border-right: 1px solid #eeeeee;">
      <yun-tag type="primary">多选</yun-tag>
      <yun-divider style="margin: 8px 0;"></yun-divider>
      <yun-tree
        :data="data"
        show-checkbox
        multiple
    ...
    <div class="p10" style="width: 300px; border-right: 1px solid #eeeeee;">
      <yun-tag type="primary">附加图标</yun-tag>
      <yun-divider style="margin: 8px 0;"></yun-divider>
      <yun-tree :data="data1"></yun-tree>
    </div>
  </div>
</template>
...
<template>
  <div class="myun-16">
    <yun-button @click="expandAll" size="small">展开全部</yun-button>
    <yun-button @click="collapseAll" size="small">收起全部</yun-button>
    <yun-button @click="setExpand" size="small">展开三级</yun-button>
    <yun-button @click="checkAll" size="small">选择全部</yun-button>
    <yun-button @click="uncheckAll" size="small">取消全选</yun-button>
    <yun-button @click="setChecked" size="small">设置选中1-2及以下</yun-button>
    <yun-button @click="setSelected" size="small">单选 1-2-1</yun-button>
    <yun-button @click="clear" size="small">清空单选和多选</yun-button>
  </div>
  <div style="width: 300px;">
    <yun-tree :data="data" show-checkbox ref="tree"></yun-tree>
  </div>
</template>
<script>
  export default {
...
<template>
  <div style="width: 300px;">
    <yun-tree :data="data3" show-checkbox :load-data="loadData"></yun-tree>
  </div>
</template>
<script>
  export default {
...
<template>
  <div>
    <yun-input
      ...
      style="width: 230px;"
    ></yun-input>
    <yun-divider style="margin: 14px 0;"></yun-divider>
    <div style="width: 300px;">
      <yun-tree :data="data" ref="treeRef" :filter-node-method="filterNode" highlight-filter></yun-tree>
    </div>
  </div>
</template>
<script>
...
<template>
  <div flex>
    <div class="p10" style="width: 300px; border-right: 1px solid #eeeeee;">
      <yun-tag type="primary">基础拖拽排序</yun-tag>
      <yun-divider style="margin: 8px 0;"></yun-divider>
      <yun-tree
        :data="data"
        ref="treeRef"
    ...
    <div class="p10" style="width: 300px; border-right: 1px solid #eeeeee;">
      <yun-tag type="primary">自定义函数配置</yun-tag>
      <yun-divider style="margin: 8px 0;"></yun-divider>
      <yun-tree
  ...
  export default {
    setup() {
      ...
      function renderContent1({ root, node, data }) {
        // 实际使用时，在最上方导入 import { YunDropdown, YunDropdownMenu, YunDropdownItem } from 'YunxiuNext' 即可
        const { YunDropdown, YunDropdownMenu, YunDropdownItem } = this.YunxiuNext
        const iconMap = {
          root: '',
          hierarchy: 'cluster',
          folder: 'folder',
        ...
        const inline = [
          h(
            ...
            [
              h('i', {
                class: ['yun-iconfont', `yun-icon-${iconMap[iconType]}`],
          ...
          h(
            ...
            {
              default: () => h('i', { class: ['yun-iconfont', 'yun-icon-setting', 'setting-action'] }),
              dropdown: () =>
                h(YunDropdownMenu, () => [
                  h(YunDropdownItem, { name: 'edit' }, () => [
                    h('i', { class: 'yun-iconfont yun-icon-edit-square' }),
                    '编辑',
                  ]),
                  h(YunDropdownItem, { name: 'delete' }, () => [
                    h('i', { class: 'yun-iconfont yun-icon-delete' }),
                    '删除',
                  ]),
                ]),
            },
...
<template>
  <div flex>
    <div class="p10" style="width: 300px; border-right: 1px solid #eeeeee;">
      <yun-tag type="primary">新增移除</yun-tag>
      <yun-divider style="margin: 8px 0;"></yun-divider>
      <yun-tree :data="data" :render="renderContent" ref="tree"></yun-tree>
    </div>
    <div class="p10" style="width: 300px; border-right: 1px solid #eeeeee;">
      <yun-tag type="primary">下拉菜单</yun-tag>
      <yun-divider style="margin: 8px 0;"></yun-divider>
      <yun-tree :data="data1" :render="renderContent1"></yun-tree>
    </div>
  </div>
</template>
<script>
  import { h } from 'vue'
  // import { YunDropdown, YunDropdownMenu, YunDropdownItem } from 'yun-ui-next'
  ...
  export default {
            ...
            render: ({ root, node, data }) => {
              return h(
                ...
                [
                  h('span', data.title),
                  h('i', {
                    class: ['yun-iconfont', 'yun-icon-plus-square-fill'],
    ...
    methods: {
        ...
        return h(
          ...
          [
            ...
            h('span', { style: { width: '36px' } }, [
              ...
              h('i', {
                class: ['yun-iconfont', 'yun-icon-minus-square-fill'],
      ...
      renderContent1({ root, node, data }) {
        // 实际使用时，在最上方导入 import { YunDropdown, YunDropdownMenu, YunDropdownItem } from 'yun-ui-next' 即可
        const { YunDropdown, YunDropdownMenu, YunDropdownItem } = this.YunxiuNext
        const inline = [
          h(
            'span',
            ...
            [h('i', { class: ['yun-iconfont', `yun-icon-${data.icon}`], style: { marginRight: '4px' } }), data.title],
          ),
          h(
            YunDropdown,
            {
  ...
  <div flex>
    <div flex>
      <yun-tree-select
        ...
        @change="handleChange"
      ></yun-tree-select>
      <yun-button @click="defaultSelect">设置默认选中前端组</yun-button>
    </div>
    <div flex class="ml-16">
      <yun-tree-select
        ...
        @change="handleChange"
      ></yun-tree-select>
      <yun-button @click="setChecked">设置默认勾选项目部</yun-button>
    </div>
  </div>
  <div>{{ currentId }}</div>
  <div>{{ currentNode }}</div>
...
如果需要渲染超大数据量的属性结构，需要使用扩展组件`<yun-big-tree>`,组件内部 api 基本复用树结构所有属性，对树形结构进行优化，利用可视区域位置来进行过滤筛选节点并进行操作，
...
<template>
  <div>
    <div style="margin-bottom: 8px;">
      数据条数：
      <yun-input-number v-model="number"></yun-input-number>
      <yun-button @click="init">渲染</yun-button>
    </div>
    <div flex="main:justify">
      ...
      ></yun-input>
      <yun-button-group>
        <yun-button @click="expandAll">展开所有</yun-button>
        <yun-button @click="collapseAll">收起所有</yun-button>
      </yun-button-group>
    </div>
    <yun-divider style="margin: 14px 0;"></yun-divider>
    <yun-big-tree
...
| timeout            | 刷新频率（`<yun-big-tree>`扩展组件可用）                                                                                                                                                           | Number                                 | —      | 17       |
| itemHeight         | 节点高度 （`<yun-big-tree>`扩展组件可用）                                                                                                                                                          | Number                                 | —      | 28       |
| visibleCount       | 显示区域个数（`<yun-big-tree>`扩展组件可用）                                                                                                                                                       | Number                                 | —      | 15       |
