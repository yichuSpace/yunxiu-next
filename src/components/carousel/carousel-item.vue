<template>
  <div
    v-show="data.ready"
    class="yun-carousel__item"
    :class="{
      'is-active': data.active,
      'yun-carousel__item--card': type === 'card',
  ...
  >
    <div v-if="type === 'card'" v-show="!data.active" class="yun-carousel__mask"></div>
    <slot></slot>
  </div>
</template>
<script>
import { reactive, onMounted, inject, computed, toRefs, getCurrentInstance } from 'vue'
import { autoprefixer } from '../../utils/utilsHelper'

const CARD_SCALE = 0.83
export default {
  name: 'YunCarouselItem',
  props: {
