<template>
  <div ref="root" :class="carouselClasses" @mouseenter.stop="handleMouseEnter" @mouseleave.stop="handleMouseLeave">
    <div class="yun-carousel__container" :style="{ height: height }">
      <transition v-if="arrowDisplay" name="carousel-arrow-left">
        <button
          v-show="(arrow === 'always' || data.hover) && (props.loop || data.activeIndex > 0)"
          type="button"
          class="yun-carousel__arrow yun-carousel__arrow--left"
      ...
      <transition v-if="arrowDisplay" name="carousel-arrow-right">
        <button
          ...
          type="button"
          class="yun-carousel__arrow yun-carousel__arrow--right"
    ...
    <ul v-if="indicatorPosition !== 'none'" :class="indicatorsClasses">
      <li
        ...
        :key="index"
        :class="[
          'yun-carousel__indicator',
          'yun-carousel__indicator--' + direction,
          { 'is-active': index === data.activeIndex },
        ]"
        @mouseenter="throttledIndicatorHover(index)"
        @click.stop="handleIndicatorClick(index)"
      >
        <button class="yun-carousel__button">
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const carouselClasses = computed(() => {
      const classes = ['yun-carousel', 'yun-carousel--' + props.direction]
      if (props.type === 'card') {
        classes.push('yun-carousel--card')
      }
      return classes
    })

    const indicatorsClasses = computed(() => {
      const classes = ['yun-carousel__indicators', 'yun-carousel__indicators--' + props.direction]
      if (hasLabel.value) {
        classes.push('yun-carousel__indicators--labels')
      }
      if (props.indicatorPosition === 'outside' || props.type === 'card') {
        classes.push('yun-carousel__indicators--outside')
      }
      return classes
    })
