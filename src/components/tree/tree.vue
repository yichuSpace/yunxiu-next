<template>
  ...
  >
    ...
    <div v-else>
      <yun-empty style="margin: 16px 0">{{ emptyText }}</yun-empty>
    </div>
    <div v-show="dragState.showDropIndicator" ref="dropIndicator$" class="yun-tree__drop-indicator"></div>
  </div>
</template>

<script>
import TreeNode from './node.vue'
import YunEmpty from '../empty'
  ...
  setup(props, ctx) {
    ...
    function updateTreeUp(nodeKey) {
      ...
      if (node.checked === true) {
        ...
        parent['indeterminate'] = parent[props.childrenKey].some(node => node.checked || node.indeterminate)
      }
    ...
    function getCheckedAndIndeterminateNodes() {
      return states.flatState.filter(obj => obj.node.checked || obj.node.indeterminate).map(obj => obj.node)
    }
    ...
    function handleTreeNodeDragOver({ e, treeNode }) {
      ...
      const iconPosition = dropNode.$el.querySelector('.yun-tree-arrow').getBoundingClientRect()
