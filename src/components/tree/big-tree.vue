<template>
  <div :class="prefixCls" ref="scrollerRef" :style="wrapStyle" @scroll="handleScroll">
      ...
      <div :class="`${prefixCls}-content`" :style="{ transform: `translateY(${offset}px)` }">
        ...
        >
          <div :class="arrowClasses(node)" @click="handleExpand(node)">
            <i v-if="node[childrenKey] && node[childrenKey].length" class="yun-iconfont yun-icon-caret-right"></i>
          </div>
          <yun-checkbox
            v-if="showCheckbox"
            :value="node.checked"
            ...
            @click.native.prevent="handleCheck(node)"
          ></yun-checkbox>
          <span
            v-if="node.display"
...
export default {
  name: 'YunBigTree',
  components: { YunEmpty, YunCheckbox },
  props: {
    data: {
      type: Array,
      default: () => {
