  ...
  <div class="nav-wrap" :class="{ 'is-scrollable': isScrollable }">
    <template v-if="!hideArrow && isScrollable">
      <span class="nav-prev" :class="{ disabled: hidePrev }" @click="handlePrev">
        <i class="yun-iconfont yun-icon-left" />
      </span>
      <span class="nav-next" :class="{ disabled: hideNext }" @click="handleNext">
        <i class="yun-iconfont yun-icon-right" />
      </span>
    </template>
    <div class="scroll-container" ref="scrollContainerRef" @wheel.prevent="handleScroll">
      <div class="scroll-wrapper" ref="scrollWrapperRef" :style="{ transform: `translateX(${left}px)` }">
        <slot></slot>
      </div>
    </div>
  </div>
...
export default {
  ...
  setup() {
    ...
    const hideNext = computed(
      () => data.left === scrollContainerRef?.value.offsetWidth - scrollWrapperRef?.value.offsetWidth - padding,
    )
    onMounted(() => {
      calcWidth()
    })
