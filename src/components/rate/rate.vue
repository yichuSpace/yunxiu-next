<template>
  ...
  >
    ...
    >
      <i
        class="yun-rate__icon yun-iconfont"
        :class="[`yun-icon-${classes[item - 1]}`, { hover: hoverIndex === item }]"
        :style="getIconStyle(item)"
      >
        <i
          class="yun-rate__decimal yun-iconfont"
          v-if="showDecimalIcon(item)"
          :class="`yun-icon-${decimalIconClass}`"
          :style="decimalStyle"
        >
        </i>
      </i>
    </span>
    <span v-if="showText || showScore" class="yun-rate__text" :style="{ color: textColor }">{{ text }}</span>
  </div>
</template>
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const rateDisabled = computed(() => props.disabled || YunForm.disabled)
    ...
    function showDecimalIcon(item) {
      let showWhenDisabled =
        rateDisabled.value && valueDecimal.value > 0 && item - 1 < props.modelValue && item > props.modelValue
      /* istanbul ignore next */
      let showWhenAllowHalf =
        props.allowHalf && pointerAtLeftHalf.value && item - 0.5 <= currentValue.value && item > currentValue.value
    ...
    function setCurrentValue(value, event) {
      ...
      if (props.allowHalf) {
        ...
        if (hasClass(target, 'yun-rate__item')) {
          target = target.querySelector('.yun-rate__icon')
        }
        if (hasClass(target, 'yun-rate__decimal')) {
