<template>
  <div :class="['yun-cascader-panel', border && 'is-bordered']" @keydown="handleKeyDown">
    <yun-cascader-menu
...
import YunCascaderMenu from './menu.vue'
...
import { arrayFlat, coerceTruthyValueToArray, deduplicate, isEmpty } from '../../../utils/utilsHelper'
...
export default {
  name: 'YunCascaderPanel',
  components: { YunCascaderMenu },
  props: {
  ...
  setup(props, { emit, slots }) {
    ...
    const scrollToExpandingNode = () => {
      ...
      menuList.value.forEach(menu => {
        ...
        if (menuElement) {
          const container = menuElement.querySelector('.yun-scrollbar__wrap')
          const activeNode =
            menuElement.querySelector('.yun-cascader-node.is-active') ||
            menuElement.querySelector('.yun-cascader-node.in-active-path')
    ...
    const handleKeyDown = e => {
      ...
      switch (code) {
        ...
        case EVENT_CODE.left:
          ...
          // eslint-disable-next-line no-case-declarations
          const expandedNode = preMenu?.$el.querySelector('.yun-cascader-node[aria-expanded="true"]')
        ...
        case EVENT_CODE.right:
          ...
          // eslint-disable-next-line no-case-declarations
          const firstNode = nextMenu?.$el.querySelector('.yun-cascader-node[tabindex="-1"]')
          focusNode(firstNode)
          break
