<template>
  <transition name="yun-notice-fade">
    <div
      v-show="visible"
      :id="id"
      :class="['yun-notice', customClass, horizontalClass]"
      :style="noticeStyles"
      role="alert"
      @mouseenter="clearTimer()"
      @mouseleave="startTimer()"
    >
      <i v-if="type || iconClass" class="yun-notice__icon" :class="[typeClass, iconClass]"></i>
      <div class="yun-notice__group" :class="{ 'is-with-icon': typeClass || iconClass }">
        <h2 class="yun-notice__title" v-text="title"></h2>
        <div v-show="message" class="yun-notice__content">
          <slot>
...
export default {
  ...
  setup(props) {
    const typeClass = computed(() => {
      ...
      return type && TypeMap[type] ? `yun-iconfont yun-icon-${TypeMap[type]} is-${type}` : ''
    })
