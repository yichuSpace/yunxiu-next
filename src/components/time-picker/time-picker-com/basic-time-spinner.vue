<template>
  <div class="yun-time-spinner" :class="{ 'has-seconds': showSeconds }">
    <template v-if="!arrowControl">
      <yun-scrollbar
        v-for="item in spinnerItems"
        :key="item"
        :ref="getRefId(item)"
        class="yun-time-spinner__wrapper"
        wrap-style="max-height: inherit;"
        view-class="yun-time-spinner__list"
        noresize
        tag="ul"
    ...
    <template v-if="arrowControl">
      <div
        v-for="item in spinnerItems"
        :key="item"
        class="yun-time-spinner__wrapper is-arrow"
        @mouseenter="emitSelectRange(item)"
      >
        <i @click="onDecreaseClick" class="yun-time-spinner__arrow yun-iconfont yun-icon-up"></i>
        <i @click="onIncreaseClick" class="yun-time-spinner__arrow yun-iconfont yun-icon-down"></i>
        <ul class="yun-time-spinner__list">
          <li
...
export default {
  ...
  setup(props, ctx) {
    ...
    const adjustSpinner = (type, value) => {
      ...
      if (el.value) {
        el.value.$el.querySelector('.yun-scrollbar__wrap').scrollTop = Math.max(0, value * typeItemHeight(type))
      }
    }
    ...
    const handleScroll = type => {
      ...
      const value = Math.min(
        Math.round(
          (listRefsMap[type].value.$el.querySelector('.yun-scrollbar__wrap').scrollTop -
    ...
    const bindScrollEvent = () => {
      const bindFuntion = type => {
        if (listRefsMap[type].value) {
          listRefsMap[type].value.$el.querySelector('.yun-scrollbar__wrap').onscroll = () => {
