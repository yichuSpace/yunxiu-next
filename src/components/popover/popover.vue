...
import YunPopper, { defaultProps, Theme } from '../popper'
...
const _hoist = { key: 0, class: 'yun-popover__title', role: 'title' }
const _content = { key: 1, class: 'yun-popover__body', role: 'body' }
export default {
  name: 'YunPopover',
  components: {
    YunPopper,
  ...
  setup(props, ctx) {
    if (process.env.NODE_ENV !== 'production' && props.visible && !ctx.slots.reference) {
      throwWarn('YunPopover', ' You cannot init popover without given reference')
    }
  ...
  render() {
    ...
    const content = $slots.content
      ? renderSlot($slots, 'content', {}, () => [createTextVNode(toDisplayString(this.content), PatchFlags.TEXT)])
    ...
    const kls = [this.content ? 'yun-popover--plain' : '', 'yun-popover', popperClass].join(' ')
